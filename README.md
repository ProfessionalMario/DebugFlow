...
Four additional scenarios live in `demo_gifs/`:

| Refracted cross-call | Crash blame chain | Data streaming | All node states |
|---|---|---|---|
| ![Refracted](demo_gifs/demo_refracted.gif) | ![Crash blame](demo_gifs/demo_crash_blame.gif) | ![Streaming](demo_gifs/demo_streaming.gif) | ![All states](demo_gifs/demo_all_states.gif) |
| A downward caller re-entering an earlier node fires a **purple** refracted pulse travelling back up. | When a function crashes, every caller in the chain is blamed and flashes red simultaneously. | Ten packets in flight at once show the non-blocking producer-consumer architecture. | Ghost (grey), processing (yellow glow), success (teal), nuke (red glow) all on one spine. |

> The demo GIFs are generated by `scripts/build_demo_gifs.py` (Pillow only, no GUI required) and written to `demo_gifs/`. Rebuild after any palette or animation tweak.
...
This installs the `debugflow` package and three console scripts: `debugflow`, `debugflow-logs`, `debugflow-logs-on`, `debugflow-logs-off`.
...
1. Press **Ctrl+Alt+F** — the translucent HUD docks to the right edge of your screen and immediately fires a **ghost scan** of your most recently modified `.py` file, populating the spine without running your real code.
2. Press **Ctrl+Alt+S** — fires a **live trace** of that same file, streaming every real call into the HUD.
...
- **Project-rooted logs** — `debugflow-logs on` writes to `<your_project>/logs/debugflow.log`, never to the package directory or `C:\Users\...`.
- **Pulse animation** — cyan downward pulse on `call`, upward pulse on `return`, red flash on `exception`, green flash on success; **purple refracted pulse** when a caller re-enters a node that is higher on the spine (cross-call detected automatically by geometry).
- **State-aware glow** — each node’s glow ring colour matches its live state: grey (ghost), yellow (processing), teal (success), red (nuke / terminated).
...
| ---------------------- | ------------------ | ---------------------------------------------------- |
| `FLOW_PROJECT_ROOT`    | (cwd of `debugflow activate`) | Directory the engine scans for `.py` files     |
...
```bash
debugflow activate           # Start sentinel + bind hotkeys
debugflow deactivate         # Stop sentinel + close HUD
debugflow status             # Show running state
debugflow                    # Show usage banner

debugflow-logs on            # Enable file logging to <project>/logs/debugflow.log
debugflow-logs off           # Disable
debugflow-logs               # Toggle current state
```
...
**Version:** 1.0.5 — usable as a daily debugging companion.
...
- To prevent ModuleNotFoundError in the HUD, always run debugflow activate from your Project Root (the same folder where you run your main script) so that relative imports resolve correctly.
